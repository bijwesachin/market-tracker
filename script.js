document.addEventListener("DOMContentLoaded", () => {
  ...
  function paintEvent(li, isoDate) {
    ...
    if (isTomorrow(isoStr = isoDate)) li.classList.add('tomorrow');
  ...
  function addEvent(listEl, ev, tzLabel = 'CT', context = '') {
    ...
    // Set the type pill text
    if (typeEl) {
      typeEl.textContent = ((ev?.type) || 'EVENT').toUpperCase();

      // NEW: light-red pill for FOMC/Fed talks and OPEX
      const redKeywords = /(fomc|fed|powell|chair|press conference|talk)/i;
      const isFedish = redKeywords.test(ev?.label || '') || redKeywords.test(ev?.type || '');
      const isOpex = /\bopex\b/i.test(ev?.label || '') || /\bopex\b/i.test(ev?.type || '');
      if (isFedish || isOpex) {
        typeEl.classList.add('alert'); // styles.css adds .event-type.alert
      }
    }
  ...
  function buildSpecials() {
    ...
    candidates.forEach((d) => {
      ...
      // Label/type used by addEvent to color the pill
      const ev = {
        date: dIso,
  ...
  // ===== Earnings (supports flat array or object map) =====
