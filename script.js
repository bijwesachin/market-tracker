document.addEventListener("DOMContentLoaded", () => {
  ...
  function fmtTime12CT(timeStr) {
    ...
    if (/^(am|pm)$/i.test(timeStr)) {
      ...
      const h24 = isAM ? 9 : 16; // 9:00 AM vs 4:00 PM CT
      const h12 = h24 % 12 || 12;
      const mm = '00';
  ...
  function addEvent(listEl, ev, tzLabel = 'CT', context = '') {
    ...
    // NEW: small red pill markers
    // Economic events: mark FOMC meetings / Fed talks
    if (context !== 'earnings') {
      const redKeywords = /(fomc|fed|powell|fed talk|chair|press conference)/i;
      if (redKeywords.test(ev?.label || '') || redKeywords.test(ev?.type || '')) {
        const redPill = document.createElement('span');
        redPill.className = 'pill pill-red';
        redPill.textContent = 'FOMC/FED';
        // place after meta block
        li.appendChild(redPill);
      }
    }
  ...
  function buildSpecials() {
    ...
    candidates.forEach((d) => {
      ...
      // NEW: mark OPEX with red pill
      if (isOpex) {
        const lastLi = ul.lastElementChild;
        if (lastLi) {
          const redPill = document.createElement('span');
          redPill.className = 'pill pill-red';
          redPill.textContent = 'OPEX';
          lastLi.appendChild(redPill);
        }
      }
    });
  ...
  // ===== Earnings (baseline expects FLAT ARRAY or already adapted; keep your format) =====
  // If your baseline uses FLAT array: [{ticker,date,time?}], normalize into {symbol,events[]} groups.
  ...
  function buildEarnings(earnRaw) {
    ...
    const groups = Array.isArray(earnRaw)
      ? normalizeEarningsFlat(earnRaw)
      : Object.keys(earnRaw || {}).map(sym => ({
          symbol: sym,
          events: (earnRaw[sym] || []).map(ev => ({
            date: ev.date, time: ev.time || '', label: '', type: ev.type || 'EARNINGS'
          }))
        }));
    ...
    groups.forEach(t => {
      ...
      if (title) title.textContent = (t.symbol || '').trim();
  ...
  async function renderAll() {
    const [econ, earnings] = await Promise.allSettled([
      ...
      getJSON(EARNINGS_JSON, []) // supports either flat array or object per normalize
    ]);
    buildSpecials();
